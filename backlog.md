...
| Teto de baratas simultâneas progressivo | Hoje o limite implícito é uma barata por comida presente (FR-021); um teto artificial menor permitiria tunar ritmo/dificuldade sem depender do nº de comidas. Entregue via `specs/013-teto-baratas-simultaneas`: `currentRoachCap(survivalMs)` substitui o limite implícito por um teto explícito que cresce de `ROACH_CAP_BASE` para `ROACH_CAP_MAX` ao longo de `DIFFICULTY_RAMP_DURATION_MS`, sempre como limite adicional à checagem de comidas disponíveis (nunca permite mais baratas que comidas presentes). Não é configurável pelo jogador nem exposto em UI/HUD — decisão explícita do dono do produto. Verificação empírica durante a implementação revelou que a concorrência natural de baratas nunca passa de 2 em nenhum ponto da rampa de dificuldade (specs/011); por isso os valores finais são `ROACH_CAP_BASE=1`/`ROACH_CAP_MAX=2` (não um range maior como 3–9, que não teria efeito observável algum). | Baixo | Médio | Baixo | ✅ **Entregue** |
